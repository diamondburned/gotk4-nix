...
let
  ...
  patchelf =
    ...
    in
    if builtins.hasAttr name pkgsHost then
      builtins.getAttr name pkgsHost
    else
      throw "no supported patchelf for target ${targetSystem}";
  ...
  buildGoApplication =
    (pkgs.callPackage "${inputs.gomod2nix}/builder" {
      inherit (pkgsCross) stdenv;
      gomod2nix = inputs.gomod2nix.packages.${pkgs.system}.default;
    }).buildGoApplication;

  builder = if (base ? modules) then buildGoApplication else buildGoModule;
...
builder {
  ...
  version = with lib; version + (optionalString (tags != [ ]) "-${concatStringsSep "+" tags}");
  ...
  nativeBuildInputs =
    (baseNativeBuildInputs pkgsHost)
    ++ (with pkgsHost; [
      pkg-config
      git # for Go
    ]);
