...
let
  ...
  targets' =
    if target != null then if builtins.isList target then target else [ target ] else targets;

  outputs = map (
    target:
    (import ./build-cross-package.nix self {
      inherit
        pkgs
        base
        tags
...
pkgs.runCommandLocal "${base.pname}-cross-${version}"
  {
    ...
    OUTPUTS = lib.concatMapStringsSep "\n" (
      ...
      in
      builtins.trace "build-cross: will make ${name}.tar.zst" "${name} ${o}"
    ) (outputs);
